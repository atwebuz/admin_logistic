...
class OrderController extends Controller
{
    public function index()
    {
        ...
        return view('pages.order.index', compact('orders', 'orderDeadlines')); // Pass orderDeadlines here
    }
       public function updateTimer($orderId)
    {
        $order = Order::findOrFail($orderId);
        $currentTime = now();
        $deadline = $order->created_at->addMinutes($order->product->category->deadline);
    
        $remainingTime = max(0, $deadline->diffInSeconds($currentTime));
